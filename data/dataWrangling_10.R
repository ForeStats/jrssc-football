...
#dir_path = path to raw data files
...
df <- data.table(rbind(data.frame(match_id = df$match_id, name = df$HT,  
                                  team = 'Home', team_id = df$Home, opp.name = df$AT),
                       data.frame(match_id = df$match_id, name = df$AT, 
                                  team = 'Away', team_id = df$Away, opp.name = df$HT)))
setkey(df, match_id)
...
my.table = data_season_1[, .N, by = c('mark')]
setkeyv(my.table, c('mark','N'))
my.table = cbind(my.table, mark_levels)
my.table = my.table[,c(1,3,2)]
my.table$mark <- as.integer(my.table$mark)
print(xtable(my.table), include.rownames = F)
